...
#         loss = 0.5 * loss_1 + 0.5 * loss_2
        ...
        loss.backward()
        optimizer.step()
#         adjust_learning_rate(optimizer, len(train_loader.dataset))
...
# def adjust_learning_rate(optimizer, n_triplets):
...
#         else:
#             group['step'] += 1.
#         group['lr'] = configs['lr'] * (1.0 - float(group['step']) * float(configs['batch_size']) / (n_triplets * float(configs['epochs'])))
#     return
...
optimizer = torch.optim.Adam(model.features.parameters(),
                             lr=configs['lr'],
                             betas=(0.9, 0.999),
                             eps=1e-08,
                             weight_decay=0,
